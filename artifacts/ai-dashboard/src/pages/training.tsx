...
export default function Training() {
  ...
  const headers = { "Content-Type": "application/json", ...authHeaders(adminKey) };
  ...
  const { data: status, isLoading: statusLoading, refetch: refetchStatus } = useGetTrainingStatus({
    query: { refetchInterval: (data) => isRunning(data?.state) ? 3000 : 10000 },
  });
  ...
  const stopMut = useMutation({
    ...
    onSuccess: () => { toast({ title: "Training stopped" }); refetchStatus(); },
    onError: () => toast({ variant: "destructive", title: "Failed to stop training" }),
  });
  ...
  const contStartMut = useMutation({
    mutationFn: () => apiFetch("/training/continuous/start", { method: "POST", headers }),
    onSuccess: () => { toast({ title: "Continuous training started" }); refetchCont(); },
    onError: () => toast({ variant: "destructive", title: "Failed to start continuous training" }),
  });

  const contStopMut = useMutation({
    mutationFn: () => apiFetch("/training/continuous/stop", { method: "POST", headers }),
    onSuccess: () => { toast({ title: "Continuous training stopped" }); refetchCont(); },
    onError: () => toast({ variant: "destructive", title: "Failed to stop continuous training" }),
  });
  ...
  const pullNowMut = useMutation({
    mutationFn: () => apiFetch("/training/puller/pull", { method: "POST", headers }),
    onSuccess: () => { toast({ title: "Data pull triggered" }); refetchPuller(); },
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          ...
          >
            {stopMut.isPending ? "Stopping..." : <><Square className="w-4 h-4 mr-2" /> Stop</>}
          </Button>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="glass-panel p-6 rounded-2xl">
            <h3 className="text-lg font-display font-semibold text-white mb-4 flex items-center gap-2">
              <Zap className={`w-5 h-5 ${isRunning(status?.state) ? 'text-amber-400 animate-pulse' : 'text-muted-foreground'}`} />
              Current Session
            </h3>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between p-2 rounded bg-white/5 text-sm">
                    ...
                    <Badge variant={isRunning(status?.state) ? "default" : "secondary"} className={isRunning(status?.state) ? "bg-amber-500/20 text-amber-400 hover:bg-amber-500/20" : ""}>
                      {status?.state || "Idle"}
                    </Badge>
                  </div>
                  <div className="flex justify-between p-2 rounded bg-white/5 text-sm">
                  ...
                  <div className="flex justify-between p-2 rounded bg-white/5 text-sm">
                    ...
                    <span className="text-white font-mono">
                      {status?.elapsed_seconds ? `${Math.floor(status.elapsed_seconds / 60)}m ${status.elapsed_seconds % 60}s` : "—"}
                    </span>
                  </div>
                  {etaFormatted && (
          ...
          <div className="glass-panel p-5 rounded-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-display font-semibold text-white flex items-center gap-2">
                <RotateCcw className={`w-4 h-4 ${contRunning ? 'text-green-400 animate-spin' : 'text-muted-foreground'}`} />
                Continuous Training
              </h3>
              <Badge variant="outline" className={contRunning ? "border-green-500/40 text-green-400" : "border-white/10 text-muted-foreground"}>
              ...
              <div className="space-y-1.5 text-xs">
                {contStatus.cycle_count != null && (
                  <div className="flex justify-between text-muted-foreground">
                    ...
                    <span className="text-white font-mono">{contStatus.cycle_count}</span>
                  </div>
                )}
                {contStatus.last_loss != null && (
                  <div className="flex justify-between text-muted-foreground">
                    <span>Last loss</span>
                    <span className="text-white font-mono">{Number(contStatus.last_loss).toFixed(4)}</span>
                  </div>
                )}
              </div>
            )}
            <div className="flex gap-2 pt-1">
              <Button size="sm" disabled={contRunning || contStartMut.isPending} onClick={() => contStartMut.mutate()}
                className="flex-1 h-8 text-xs bg-green-600/20 hover:bg-green-600/30 text-green-300 border border-green-600/30">
                <Play className="w-3 h-3 mr-1" /> Start
              </Button>
              <Button size="sm" disabled={!contRunning || contStopMut.isPending} onClick={() => contStopMut.mutate()}
          ...
          <div className="glass-panel p-5 rounded-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-display font-semibold text-white flex items-center gap-2">
                <Database className={`w-4 h-4 ${pullerRunning ? 'text-blue-400 animate-pulse' : 'text-muted-foreground'}`} />
                Data Puller
              </h3>
              <Badge variant="outline" className={pullerRunning ? "border-blue-500/40 text-blue-400" : "border-white/10 text-muted-foreground"}>
              ...
              <div className="space-y-1.5 text-xs">
                {pullerStatus.samples_pulled != null && (
                  <div className="flex justify-between text-muted-foreground">
                    ...
                    <span className="text-white font-mono">{Number(pullerStatus.samples_pulled).toLocaleString()}</span>
                  </div>
                )}
                {pullerStatus.last_pull_at && (
                  <div className="flex justify-between text-muted-foreground">
                    <span>Last pull</span>
                    <span className="text-white font-mono">{format(new Date(pullerStatus.last_pull_at), "HH:mm:ss")}</span>
                  </div>
                )}
              </div>
            )}
            <Button size="sm" disabled={pullNowMut.isPending} onClick={() => pullNowMut.mutate()}
              className="w-full h-8 text-xs bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-600/30">
              <RefreshCw className={`w-3 h-3 mr-1 ${pullNowMut.isPending ? 'animate-spin' : ''}`} />
