  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      <div className="text-center max-w-2xl mx-auto mb-10">
        ...
        <h1 className="text-4xl font-display font-bold text-white mb-3">Content Generator</h1>
        <p className="text-muted-foreground text-lg">Harness the AI model to generate highly optimized scripts and captions across 8 social platforms.</p>
      </div>
          ...
          <Card className="glass-panel p-6 border-white/10">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 pt-2">
                <label className="text-sm font-medium text-muted-foreground">Topic / Idea</label>
                <Textarea 
          ...
          <AnimatePresence mode="wait">
            {!result && !generateMut.isPending ? (
              ...
              >
                ...
                <p className="animate-pulse font-medium">Model is thinking...</p>
              </motion.div>
            ) : result && (
              <motion.div 
              ...
              >
                <div className="flex justify-between items-center border-b border-white/10 pb-4">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    <span className="capitalize text-white font-medium bg-white/10 px-2 py-1 rounded">{result.platform}</span>
                    <span>•</span>
                    <span>{result.processing_time_ms.toFixed(0)}ms</span>
                    <span>•</span>
                    <span className="text-primary-foreground">{result.source}</span>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => copyText(result.caption)}>
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-primary mb-2">Hook</h4>
                    <p className="text-lg text-white font-medium">{result.hook}</p>
                  </div>
                  
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2">Body</h4>
                    <p className="text-gray-300 whitespace-pre-wrap leading-relaxed">{result.body}</p>
                  </div>

                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-amber-400 mb-2">Call to Action</h4>
                  ...
                  <div className="pt-4 border-t border-white/10">
                    <div className="flex flex-wrap gap-2">
                      {result.hashtags.map(tag => (
                        <span key={tag} className="text-sm text-blue-400 bg-blue-500/10 px-2 py-1 rounded border border-blue-500/20">
                          {tag}
                        </span>
                      ))}
                    </div>
