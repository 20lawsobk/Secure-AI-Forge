...
transformer model using control-token conditioning.  The agent selects the
rendering template by mapping the model-determined tone and genre to one of the
23 cinematic templates in templates_v2.  Templates are purely rendering engines;
zero pre-written content flows in from this file.
"""
...
from .renderer import PLATFORM_RATIOS
...
class VideoAgent:
    ...
    def plan(self, req: VideoAgentRequest) -> VideoProduction:
        ...
        template_id = self._select_template(tone, genre)
    ...
    def render(self, req: VideoAgentRequest) -> CinematicResult:
        """Full pipeline: plan → cinematic_request → rendered MP4."""
        production = self.plan(req)
        cinematic_req = self._build_cinematic_request(req, production)
        return render_cinematic(cinematic_req)
